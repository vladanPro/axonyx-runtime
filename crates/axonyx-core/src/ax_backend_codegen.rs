...
pub fn generate_backend_module(plan: &AxBackendPlan) -> Result<String, AxBackendCodegenError> {
    ...
    for function in &plan.functions {
        out.push_str(&render_function_fn(function, &context)?);
        out.push('\n');
    }
    ...
    for handler in &plan.handlers {
        ...
        out.push_str(&render_handler_fn(handler, &plan.globals, &context)?);
        out.push('\n');
    }
...
fn render_function_fn(
    ...
    let params = function
        ...
        .iter()
        .map(|field| {
            let rust_ty = if context.record_names.contains(&field.rust_ty) {
                format!("&{}", field.rust_ty)
            } else {
                field.rust_ty.clone()
            };
            format!("{}: {rust_ty}", field.name)
        })
        .collect::<Vec<_>>()
        .join(", ");
...
fn render_function_step(
    ...
    match step {
        AxStepPlan::Let { binding, value } => Ok(format!(
            "    let {binding} = {};\n",
            render_function_value_plan(value, function, binding, context)?
        )),
...
fn render_declared_function_call(
    path: &[String],
    args: &[AxRustExpr],
    functions: &[AxFunctionPlan],
    record_names: &std::collections::BTreeSet<String>,
) -> String {
    let [name] = path else {
        return render_call_expr(path, args);
    };
    ...
    let args = args
        ...
        .enumerate()
        .map(|(index, arg)| {
            let rendered = render_owned_expr(arg);
            if function
                .input
                .get(index)
                .is_some_and(|field| record_names.contains(&field.rust_ty))
            {
                format!("&{rendered}")
            } else {
                rendered
...
"#,
    ...
    #[test]
    fn borrows_record_arguments_for_policy_helpers_after_optional_narrowing() {
        let module = compile_backend_ax_to_module(
            r#"
type User {
  id: String
