...
assert(state.diagnostics().wasmOperations === 1, "structured state did not execute through WASM");
assert(state.get("page:posts:items:1")[0].title === "First", "record list changed in transport");

console.log("Axonyx typed state bridge passed (List<Optional<Post>> via WASM ABI 3).");
